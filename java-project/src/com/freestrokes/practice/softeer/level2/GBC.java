 ...
 *
 * 출처:
 * https://www.softeer.ai/practice/info.do?idx=1&eid=584
 *
 * 참고:
 * https://void2017.tistory.com/363
 * https://serylee.tistory.com/29
 *
 */
public class GBC {

    public static int n, m, sectionLength = 0, testSectionLength = 0, result = 0;
    public static int[] lengthArr, limitArr, testLengthArr, testLimitArr;
...
//
//        n = Integer.parseInt(st.nextToken()); // 입력 받을 기준 구간과 제한 속도의 개수
//        m = Integer.parseInt(st.nextToken()); // 입력 받을 테스트 기준 구간과 제한 속도의 개수
//
//        lengthArr = new int[n];
//        limitArr = new int[n];
//
//        testLengthArr = new int[m];
//        testLimitArr = new int[m];

        // TODO
        n = 3;
        m = 3;

        lengthArr = new int[]{50, 40, 10};
        limitArr = new int[]{50, 40, 30};

        testLengthArr = new int[]{60, 18, 22};
        testLimitArr = new int[]{76, 28, 50};
...
//            limitArr[i] = Integer.parseInt(st.nextToken());
//        }
//
//        // 테스트 기준 구간과 제한 속도
//        for (int j = 0; j < m; j++) {
//            st = new StringTokenizer(br.readLine());
//            testLengthArr[j] = Integer.parseInt(st.nextToken());
//            testLimitArr[j] = Integer.parseInt(st.nextToken());
//        }

        for (int i = 0; i < n; i++) {
            sectionLength += lengthArr[i];

            for (int j = 0; j < m; j++) {
                testSectionLength += testLengthArr[j];
//                System.out.println("test length: " + testSectionLength + " / section length: " + sectionLength);
//
                if (testSectionLength > sectionLength) {
//                    System.out.println("result: " + result + " / exceed: " + (testLimitArr[i] - limitArr[i]));
//                    result = Math.max(result, testLimitArr[i] - limitArr[i]);
                } else if (testSectionLength <= sectionLength) {
                    result = Math.max(result, testLimitArr[i] - limitArr[i]);
                }
            }
        }
        ...
        // TODO
        System.out.println("lengthArr: " + Arrays.toString(lengthArr));
        System.out.println("limitArr: " + Arrays.toString(limitArr));
        System.out.println("testLengthArr: " + Arrays.toString(testLengthArr));
        System.out.println("testLimitArr: " + Arrays.toString(testLimitArr));
